...
const ExpenseCategory: React.FC = () => {
  ...
  const [addForm, setAddForm] = useState({ name: '', description: '', status: true });

  const [showEditModal, setShowEditModal] = useState(false);
  const [editForm, setEditForm] = useState({ id: 0, name: '', description: '', status: true });
  ...
  const handleAdd = async () => {
    try { const res = await createExpenseCategory({ name: addForm.name, description: addForm.description, status: addForm.status ? 'active' : 'inactive' }); setCategories(prev => [res.data, ...prev]); } catch {}
    setAddForm({ name: '', description: '', status: true }); setShowAddModal(false);
  };

  const openEditModal = (c: ExpenseCategoryType) => {
    setEditForm({ id: c.id, name: c.name, description: c.description, status: c.status === 'active' }); setShowEditModal(true);
  };

  const handleEditSave = async () => {
    try { await updateExpenseCategory(editForm.id, { name: editForm.name, description: editForm.description, status: editForm.status ? 'active' : 'inactive' }); } catch {}
    setCategories(prev => prev.map(c => c.id === editForm.id ? { ...c, name: editForm.name, description: editForm.description, status: editForm.status ? 'active' : 'inactive' } : c));
  ...
  const filtered = categories.filter(c => {
    const matchSearch = !searchTerm || c.name.toLowerCase().includes(searchTerm.toLowerCase()) || c.description.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <>
      <div className="page-header">
        <div className="add-item d-flex"><div className="page-title"><h4 className="fw-bold">Expense Category</h4><h6>Manage your expense categories</h6></div></div>
        ...
        <div className="page-btn"><a href="#" className="btn btn-primary" onClick={e => { e.preventDefault(); setShowAddModal(true); }}><i className="ti ti-circle-plus me-1"></i>Add Category</a></div>
      </div>
                ...
                <tbody>
                  {filtered.map(cat => (
                    <tr key={cat.id}>
                      ...
                      <td><span className={`badge fw-medium fs-10 ${cat.status === 'active' ? 'bg-success' : 'bg-danger'}`}>{cat.status === 'active' ? 'Active' : 'Inactive'}</span></td>
      ...
      {showAddModal && (
        <div className="modal fade show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }} tabIndex={-1}>
          <div className="modal-dialog modal-dialog-centered"><div className="modal-content">
            <div className="modal-header"><div className="page-title"><h4>Add Expense Category</h4></div><button type="button" className="close bg-danger text-white fs-16" onClick={() => setShowAddModal(false)}><span>&times;</span></button></div>
            <div className="modal-body">
              <div className="mb-3"><label className="form-label">Category<span className="text-danger ms-1">*</span></label><input type="text" className="form-control" value={addForm.name} onChange={e => setAddForm(p => ({ ...p, name: e.target.value }))} autoFocus /></div>
              ...
              <div className="mb-0"><div className="status-toggle modal-status d-flex justify-content-between align-items-center"><span className="status-label">Status</span><input type="checkbox" id="add-ec-status" className="check" checked={addForm.status} onChange={e => setAddForm(p => ({ ...p, status: e.target.checked }))} /><label htmlFor="add-ec-status" className="checktoggle"></label></div></div>
            </div>
            <div className="modal-footer"><button type="button" className="btn me-2 btn-secondary" onClick={() => setShowAddModal(false)}>Cancel</button><button type="button" className="btn btn-primary" onClick={handleAdd} disabled={!addForm.name.trim()}>Add Category</button></div>
          </div></div>
        </div>
      )}
      ...
      {showEditModal && (
        <div className="modal fade show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }} tabIndex={-1}>
          <div className="modal-dialog modal-dialog-centered"><div className="modal-content">
            <div className="modal-header"><div className="page-title"><h4>Edit Expense Category</h4></div><button type="button" className="close bg-danger text-white fs-16" onClick={() => setShowEditModal(false)}><span>&times;</span></button></div>
            <div className="modal-body">
              <div className="mb-3"><label className="form-label">Category<span className="text-danger ms-1">*</span></label><input type="text" className="form-control" value={editForm.name} onChange={e => setEditForm(p => ({ ...p, name: e.target.value }))} /></div>
              ...
              <div className="mb-0"><div className="status-toggle modal-status d-flex justify-content-between align-items-center"><span className="status-label">Status</span><input type="checkbox" id="edit-ec-status" className="check" checked={editForm.status} onChange={e => setEditForm(p => ({ ...p, status: e.target.checked }))} /><label htmlFor="edit-ec-status" className="checktoggle"></label></div></div>
            </div>
