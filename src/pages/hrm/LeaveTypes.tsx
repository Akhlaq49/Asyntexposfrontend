...
const LeaveTypes: React.FC = () => {
  ...
  const [filtered, setFiltered] = useState<LeaveType[]>([]);
  ...
  const emptyForm: CreateLeaveType = { name: '', quota: 12, status: 'active', isActive: true };
  const [form, setForm] = useState<CreateLeaveType>({ ...emptyForm });
  ...
  const loadData = async () => {
    setLoading(true);
    try { setItems(await getLeaveTypes()); } catch { showError(t('hrm.failed_load_leave_types')); }
  ...
  const openEditModal = (item: LeaveType) => {
    ...
    setForm({ name: item.name, quota: item.quota, status: item.status, isActive: item.isActive });
    setShowModal(true);
  };

  const handleSave = async () => {
    if (!form.name) { showError(t('hrm.leave_type_name_required')); return; }
    try {
      if (editingId) await updateLeaveType(editingId, form);
      else await createLeaveType(form);
      setShowModal(false); showSuccess(editingId ? t('hrm.leave_type_updated') : t('hrm.leave_type_created')); loadData();
  ...
  return (
    <>
      <div className="page-header">
        <div className="add-item d-flex"><div className="page-title"><h4>{t('hrm.leave_types')}</h4><h6>{t('hrm.manage_leave_types')}</h6></div></div>
        <div className="page-btn"><a href="#" className="btn btn-primary" onClick={e => { e.preventDefault(); openAddModal(); }}><i className="ti ti-circle-plus me-1"></i>{t('hrm.add_leave_type')}</a></div>
      </div>
              ...
              <table className="table datanew">
                <thead>
                  <tr>
                    ...
                    <th>{t('hrm.leave_type')}</th><th>{t('hrm.leave_quota')}</th><th>{t('common.created_on')}</th><th>{t('common.status')}</th><th className="text-center">{t('common.actions')}</th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0 ? <tr><td colSpan={6} className="text-center py-4">{t('hrm.no_leave_types_found')}</td></tr> : paginated.map(item => (
                    <tr key={item.id}>
        ...
        <div className="modal fade show d-block" tabIndex={-1} style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header"><h4 className="modal-title">{editingId ? t('hrm.edit_leave_type') : t('hrm.add_leave_type')}</h4><button type="button" className="close" onClick={() => setShowModal(false)}><span>&times;</span></button></div>
              <div className="modal-body">
                <div className="mb-3"><label className="form-label">{t('hrm.leave_type')}<span className="text-danger ms-1">*</span></label><input type="text" className="form-control" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
                <div className="mb-3"><label className="form-label">{t('hrm.leave_quota')}</label><input type="number" className="form-control" value={form.quota} onChange={e => setForm({ ...form, quota: parseInt(e.target.value) || 0 })} /></div>
