  ...
  return (
    <>
      <div className="page-header">
        <div className="add-item d-flex"><div className="page-title"><h4>{t('hrm.departments')}</h4><h6>{t('hrm.manage_departments')}</h6></div></div>
              ...
              <table className="table datanew">
                <thead>
                  <tr>
                    ...
                    <th>{t('hrm.department')}</th><th>{t('hrm.hod')}</th><th>{t('hrm.members')}</th><th>{t('common.created_on')}</th><th>{t('common.status')}</th><th className="text-center">{t('common.actions')}</th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0 ? <tr><td colSpan={7} className="text-center py-4">{t('hrm.no_departments_found')}</td></tr> : paginated.map(item => (
                    <tr key={item.id}>
        ...
        <div className="modal fade show d-block" tabIndex={-1} style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header"><h4 className="modal-title">{editingId ? t('hrm.edit_department') : t('hrm.add_department')}</h4><button type="button" className="close" onClick={() => setShowModal(false)}><span>&times;</span></button></div>
              <div className="modal-body">
                <div className="mb-3"><label className="form-label">{t('hrm.department_name')}<span className="text-danger ms-1">*</span></label><input type="text" className="form-control" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
                <div className="mb-3"><label className="form-label">{t('hrm.hod')}</label>
                  <select className="form-select" value={form.hodId || ''} onChange={e => setForm({ ...form, hodId: e.target.value ? parseInt(e.target.value) : undefined })}>
                    <option value="">{t('hrm.select_hod')}</option>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content p-5">
              <div className="modal-body text-center p-0">
                ...
                <h4>{t('hrm.delete_department')}</h4><p className="text-muted">{t('hrm.confirm_delete_department')}</p>
