...
const Shift: React.FC = () => {
  ...
  const emptyForm: CreateShift = { name: '', startTime: '09:00', endTime: '17:00', weekOff: 'Sunday', status: 'active', isActive: true };
  const [form, setForm] = useState<CreateShift>({ ...emptyForm });
  ...
  const openEditModal = (item: ShiftType) => {
    setEditingId(item.id);
    setForm({ name: item.name, startTime: item.startTime || '09:00', endTime: item.endTime || '17:00', weekOff: item.weekOff || 'Sunday', status: item.status, isActive: item.isActive });
    setShowModal(true);
  };
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    await deleteShift(deleteId); setShowDeleteModal(false); setDeleteId(null); showSuccess(t('hrm.shift_deleted')); loadData();
  };
