...
const LeavesAdmin: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setShowModal(false); showSuccess(editingId ? t('hrm.leave_updated') : t('hrm.leave_created')); loadData();
    } catch { showError(t('hrm.failed_save')); }
  };

  const confirmDelete = async () => {
    if (!deleteId) return;
    try { await deleteLeave(deleteId); setShowDeleteModal(false); setDeleteId(null); showSuccess(t('hrm.leave_deleted')); loadData(); }
  ...
  return (
    <>
      <div className="page-header">
        <div className="add-item d-flex"><div className="page-title"><h4>{t('hrm.leaves_admin')}</h4><h6>{t('hrm.manage_all_leaves')}</h6></div></div>
        <div className="page-btn"><a href="#" className="btn btn-primary" onClick={e => { e.preventDefault(); openAddModal(); }}><i className="ti ti-circle-plus me-1"></i>{t('hrm.add_leave')}</a></div>
      </div>
              ...
              <table className="table datanew">
                <thead>
                  <tr>
                    ...
                    <th>{t('hrm.employee')}</th><th>{t('hrm.leave_type')}</th><th>{t('hrm.from')}</th><th>{t('hrm.to')}</th><th>{t('hrm.days')}</th><th>{t('hrm.applied_on')}</th><th>{t('common.status')}</th><th className="text-center">{t('common.actions')}</th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0 ? <tr><td colSpan={9} className="text-center py-4">{t('hrm.no_leaves_found')}</td></tr> : paginated.map(item => (
                    <tr key={item.id}>
        ...
        <div className="modal fade show d-block" tabIndex={-1} style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header"><h4 className="modal-title">{editingId ? t('hrm.edit_leave') : t('hrm.add_leave')}</h4><button type="button" className="close" onClick={() => setShowModal(false)}><span>&times;</span></button></div>
              <div className="modal-body">
                <div className="mb-3"><label className="form-label">{t('hrm.employee')}<span className="text-danger ms-1">*</span></label>
                ...
                <div className="row">
                  <div className="col-lg-6"><div className="mb-3"><label className="form-label">{t('common.from_date')}</label><input type="date" className="form-control" value={form.fromDate.split('T')[0]} onChange={e => { const f = e.target.value; setForm({ ...form, fromDate: f, days: calcDays(f, form.toDate, form.dayType) }); }} /></div></div>
                  <div className="col-lg-6"><div className="mb-3"><label className="form-label">{t('common.to_date')}</label><input type="date" className="form-control" value={form.toDate.split('T')[0]} onChange={e => { const t = e.target.value; setForm({ ...form, toDate: t, days: calcDays(form.fromDate, t, form.dayType) }); }} /></div></div>
                </div>
                <div className="row">
                ...
                <div className="mb-3"><label className="form-label">{t('common.status')}</label>
                  <select className="form-select" value={form.status} onChange={e => setForm({ ...form, status: e.target.value })}>
                    <option value="New">{t('hrm.new_status')}</option><option value="Approved">{t('hrm.approved')}</option><option value="Rejected">{t('hrm.rejected')}</option>
                  </select>
                </div>
              </div>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content p-5">
              <div className="modal-body text-center p-0">
                ...
                <h4>{t('hrm.delete_leave')}</h4><p className="text-muted">{t('hrm.confirm_delete_leave')}</p>
